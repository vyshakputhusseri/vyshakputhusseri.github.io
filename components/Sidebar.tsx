...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, activeTool, setActiveTool, isOpen = false }) => {
  const menuItems = [
  ...
  return (
    <aside
      className={`sidebar-desktop w-64 h-screen fixed left-0 top-0 border-r border-[var(--border-main)] flex flex-col bg-[var(--bg-main)] overflow-y-auto z-40 transition-all duration-300 ${isOpen ? 'sidebar-open' : ''}`}
      role="navigation"
    ...
    >
      <div className="p-8">
        ...
        <nav className="space-y-2" aria-label="Site sections">
          {menuItems.map((item) => (
      ...
      <div className="mt-auto p-8 space-y-8">
        <div className="flex gap-6 px-4">
          <a href="https://github.com/vyshakputhusseri" target="_blank" rel="noopener noreferrer" aria-label="GitHub profile" className="text-[var(--text-main)] opacity-60 hover:opacity-100 hover:text-[var(--primary)] transition-all duration-300 transform hover:scale-125">
          ...
          <a href="https://www.linkedin.com/in/vyshakputhusseri" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn profile" className="text-[var(--text-main)] opacity-60 hover:opacity-100 hover:text-[var(--primary)] transition-all duration-300 transform hover:scale-125">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.761 0 5-2.239 5-5v-14c0-2.761-2.239-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
          </a>
        </div>

        <div className="px-4 text-[10px] text-[var(--text-muted)] font-medium opacity-60">
          &copy; {new Date().getFullYear()} Vyshak Puthusseri
        </div>
      </div>
    </aside>
  );
